...
async def sync_user_repos(db: AsyncSession, user_id: uuid.UUID, repos: list[OAuthRepo]) -> dict:
    ...
    existing = (
        (await db.execute(select(Repository).where(Repository.github_repo_id.in_(incoming_ids))))
        .scalars()
        .all()
    )
